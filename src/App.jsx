...
export default function App() {
  ...
  const handleAddQA = async (e) => {
    ...
    try {
      ...
      setQuestion(""); setAnswerText(""); setAnswerAudio(null);
    } catch (err) {
      console.error(err);
  ...
  const cancelEdit = () => {
    setEditId(null);
    setEditQuestion(""); setEditAnswerText("");
    setEditAnswerAudio(null); setEditExistingAudioUrl(null);
  };
  ...
  const handleSaveIntro = async (e) => {
    ...
    try {
      ...
      sequence.forEach((step, i) => {
        if (step.type === "text") formData.append(`step${i}_content`, step.content);
        else if (step.file) formData.append(`step${i}_file`, step.file);
      });
              ...
              <div className="content-wrapper">
                ...
                {editId && (
                  <form onSubmit={handleUpdateQA} className="form-card edit-form">
                    ...
                    {editExistingAudioUrl && <audio controls src={resolveUrl(editExistingAudioUrl)} />}
                    ...
                    <div className="form-actions">
                      ...
                      <button type="button" className="btn gray" onClick={cancelEdit}>Cancel</button>
                    </div>
                  </form>
                )}
                ...
                <form onSubmit={handleSaveIntro} className="form-card">
                  ...
                  <input type="file" accept="video/*" onChange={(e) => handleFileChange(0, e.target.files[0])} />
                  <input type="file" accept="video/*" onChange={(e) => handleFileChange(1, e.target.files[0])} />
                  <textarea value={sequence[4].content} onChange={(e) => handleTextChange(4, e.target.value)} placeholder="Payment details" />
                  <input type="file" accept="audio/*" onChange={(e) => handleFileChange(5, e.target.files[0])} />
                ...
                <div className="form-card">
                  ...
                  {intro.length === 0 ? <p>No intro uploaded yet.</p> : (
                    <ul className="qa-list">
                      {intro.map((step, i) => (
                        <li key={i} className="qa-card">
                          <p><b>Step {i + 1}:</b> {step.type}</p>
                          {step.type === "text" && <p>{step.content}</p>}
                          {step.type === "video" && step.fileUrl && <video controls src={resolveUrl(step.fileUrl)} />}
                          {step.type === "audio" && step.fileUrl && <audio controls src={resolveUrl(step.fileUrl)} />}
                          <div className="qa-actions">
                            <button className="btn red" onClick={() => handleDeleteIntro(i)}>Delete</button>
                          </div>
                        </li>
                      ))}
                    </ul>
                ...
                <div className="form-card">
                  ...
                  <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone e.g. +234..." />
                  ...
                  {receipts.map((order) => (
                    <div key={order._id} className="qa-card">
                      ...
                      <p><a href={order.receiptUrl} target="_blank">View Receipt</a></p>
                    </div>
                  ))}
                </div>
              </div>
